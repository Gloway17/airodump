        ...
        switch (tag_num) {
            ...
            case 0x30: // RSN Information (ENC, CIPHER, AUTH)
                if (tag_len >= 4) {
                    ...
                    uint8_t cipher_offset = offset + 8; // Pairwise Cipher Suite starts at offset 8
                    if (cipher_offset < length) {
                        uint32_t cipher_suite = tagged_parameters[cipher_offset];
                        if (cipher_suite == 0x000FAC04) {
                            PacketInfo.CIPHER = 1; // CCMP (AES)
                        } else if (cipher_suite == 0x000FAC02) {
                    ...
                    uint8_t auth_offset = offset + 14; // AKM Suite starts at offset 14
                    if (auth_offset < length) {
                        uint32_t auth_suite = tagged_parameters[auth_offset];
                        if (auth_suite == 0x000FAC02) {
                            PacketInfo.AUTH = 1; // PSK
                        } else if (auth_suite == 0x000FAC01) {
...
int airodump_print(uint8_t ch, struct packet_info *PacketInfo) {
    ...
    printf("[ CH %2d ][ Elapsed:  0 s ][ %04d-%02d-%02d %02d:%02d:%02d ]\n\n", 
            ch, 
    ...
    for (int i = 0; i < ap_count; i++) {
        printf(" %02x:%02x:%02x:%02x:%02x:%02x  %d  %7d    %5d  %3d  %2d  %3d   %s %s   %s %s\n",
